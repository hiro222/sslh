//障害物回避　回り込みプログラム　RightAttckのみ有効9/13 clang-format
#include <cmath>
...
　　// 自分の情報がなければ終了
  ...
  Eigen::Vector2d ene_robot_pos(0, 0);　// t敵ロボット２次元ベクトル設定
  ...
  Eigen::Vector2d p1, p2, leftP, rightP, target0; // ２次元ベクトル設定
  ...
  constexpr double rot_th = 0.5;
   ...
   if(e_goal_ball_x >= e_goal_robo_x){  // 回り込み判別　敵ゴールとボールの距離ｘ>=　敵ゴールと自ロボットの距離ｘ　何故敵ゴールは全て上なのか？
  　// 敵ゴールとボールの左右に任意の長さ分ずらした2点p1、P2の距離の比較
